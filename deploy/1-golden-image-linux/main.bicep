// =============================================================================
// STEP 1 (LINUX) - CrowdStrike Falcon Golden Image (self-contained)
// =============================================================================
// Creates EVERYTHING needed to build the Linux golden image:
//   - Key Vault (RBAC) seeded with the CrowdStrike secrets you pass in securely
//   - Azure Compute Gallery + image definition (Linux, Trusted Launch)
...
//
// "Deploy to Azure" button (Linux, Step 1) entry point -> compiled to
// deploy/1-golden-image-linux/azuredeploy.json
// =============================================================================
...
@minLength(3)
@maxLength(24)
param keyVaultName string = take('kvcsl${uniqueString(resourceGroup().id)}', 24)
...
param galleryName string = '${projectName}_${environment}_linux_${uniqueString(resourceGroup().id)}'

@description('Image definition name.')
param imageDefinitionName string = 'CrowdStrike-Ubuntu-2204'
...
param sourceImagePublisher string = 'Canonical'

@description('Source image offer.')
param sourceImageOffer string = '0001-com-ubuntu-server-jammy'

@description('Source image SKU (Gen2 / Trusted Launch capable).')
param sourceImageSku string = '22_04-lts-gen2'

@description('Source image version.')
param sourceImageVersion string = 'latest'

var namePrefix = '${projectName}-${environment}-linux'
var resourceTags = {
...
module imageBuilder '../../modules/imageBuilderLinux.bicep' = {
